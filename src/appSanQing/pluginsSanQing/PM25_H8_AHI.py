...
from IDLApps.SanQing.PM25_H8_AHI.PM25_H8_AHI import get_sample_data, rf_model_fit, rf_model_predict, \
    rf_pred_accuracy_scatter


class PM25_H8_AHI(BaseProcess):
    '''
    H8_AHI产品
    '''
    ...
    def doInnerPy(self):
        try:
            ...
            if BaseFile.isFileOrDir(model_file) == BaseFile.ISFILE:
                pm25, XSize, YSize, geotrans, proj = rf_model_predict(issue, model_file, fileShp, curARPFile,
                ...
                pm25[pm25 < 0] = np.NAN
                step = 2
                for i in range(step, pm25.shape[0] - step):
                    for j in range(step, pm25.shape[1] - step):
                        pm25[i, j] = np.nanmean(pm25[i - step:i + step, j - step:j + step])

                pm25[np.isnan(pm25)] = fillValue

                outFilePath = self.tempFolder + "/" + issue + ".tif"
                GeoProcessUtil.write_tiff(pm25, XSize, YSize, 1, geotrans, proj, out_path=outFilePath,
                                          no_data_value=fillValue)

                # rf_pred_accuracy_scatter(issue, pm25, geotrans, curSTNFile, stn_features, pred_scatter)
    ...
    def updateStaMapInfo(self, staMapInfo):
        ...
        issue = self.pluginParam.getIssue()
        if len(staMapRe) > 0:
            self.outStaMap["3clear_cluster_schedule_pm25_zonal_histogram"] = {
                "field": "\'issue\', \'region_id\', \'level\', \'value\'", "values": []}
            outStaMapReValues = self.outStaMap["3clear_cluster_schedule_pm25_zonal_histogram"]["values"]
        ...
        if len(staMap) > 0:
            self.outStaMap["3clear_cluster_schedule_pm25_zonal_statistics"] = {
                "field": "\'issue\', \'region_id\', \'MAX\', \'MEAN\', \'MIN\'", "values": []}
            outStaMapValues = self.outStaMap["3clear_cluster_schedule_pm25_zonal_statistics"]["values"]
