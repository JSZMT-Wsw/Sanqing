...
class O3_S5P_TROPOMI_COMPOSE(BaseProcess):
    '''
    H8_AHI合成产品
    '''
    ...
    def updateStaMapInfo(self, staMapInfo):
        ...
        if len(staMapRe) > 0:
            self.outStaMap["3clear_cluster_schedule_o3_compose_zonal_histogram"] = {
                "field": "\'issue\', \'region_id\', \'level\', \'value\'", "values": []}
            outStaMapReValues = self.outStaMap["3clear_cluster_schedule_o3_compose_zonal_histogram"]["values"]
        ...
        if len(staMap) > 0:
            self.outStaMap["3clear_cluster_schedule_o3_compose_zonal_statistics"] = {
                "field": "\'issue\', \'region_id\', \'MAX\', \'MEAN\', \'MIN\'", "values": []}
            outStaMapValues = self.outStaMap["3clear_cluster_schedule_o3_compose_zonal_statistics"]["values"]
