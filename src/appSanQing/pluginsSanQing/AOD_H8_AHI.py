...
class AOD_H8_AHI(BaseProcess):
    ...
    def doInnerPy(self):
        try:
            ...
            H8L2Obj = H8L2Process(inputFile, 'ARP', fillValue, self.tempFolder, outMark="TIF")
            outFilePath = H8L2Obj.doPrase()[0]
    ...
    def updateStaMapInfo(self, staMapInfo):
        ...
        if len(staMapRe) > 0:
            self.outStaMap["htht_cluster_schedule_aod_zonal_histogram"] = {
                "field": "\'issue\', \'region_id\', \'level\', \'value\'", "values": []}
            outStaMapReValues = self.outStaMap["htht_cluster_schedule_aod_zonal_histogram"]["values"]
        ...
        if len(staMap) > 0:
            self.outStaMap["htht_cluster_schedule_ndvi_zonal_statistics"] = {
                "field": "\'issue\', \'region_id\', \'MAX\', \'MEAN\', \'MIN\'", "values": []}
            outStaMapValues = self.outStaMap["htht_cluster_schedule_ndvi_zonal_statistics"]["values"]
