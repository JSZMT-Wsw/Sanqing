...
class NO2_S5P_TROPOMI(BaseProcess):
    '''
    哨兵5产品
    '''
    ...
    def doInnerPy(self):
        try:
            ...
            S5PL2ProcessObj = S5PL2Process(issue, inputFile, "NO2", model, fillValue, shpPath, self.tempFolder)
            outFilePath = S5PL2ProcessObj.doPrase()
            # outFilePath = r"D:\SanQing\Proj\Production\NO2_S5P_TROPOMI\202006\202006110000_COOD\000000000000\000000000000.tif"
    ...
    def updateStaMapInfo(self, staMapInfo):
        ...
        issue = self.pluginParam.getIssue()
        if len(staMapRe) > 0:
            self.outStaMap["3clear_cluster_schedule_no2_zonal_histogram"] = {
                "field": "\'issue\', \'region_id\', \'level\', \'value\'", "values": []}
            outStaMapReValues = self.outStaMap["3clear_cluster_schedule_no2_zonal_histogram"]["values"]
        ...
        if len(staMap) > 0:
            self.outStaMap["3clear_cluster_schedule_no2_zonal_statistics"] = {
                "field": "\'issue\', \'region_id\', \'MAX\', \'MEAN\', \'MIN\'", "values": []}
            outStaMapValues = self.outStaMap["3clear_cluster_schedule_no2_zonal_statistics"]["values"]
