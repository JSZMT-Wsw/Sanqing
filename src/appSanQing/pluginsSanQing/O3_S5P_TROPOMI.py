...
class O3_S5P_TROPOMI(BaseProcess):
    '''
    哨兵5产品
    '''
    ...
    def doInnerPy(self):
        try:
            ...
            S5PL2ProcessObj = S5PL2Process(issue, inputFile, "O3", model, fillValue, shpPath, self.tempFolder)
    ...
    def updateStaMapInfo(self, staMapInfo):
        ...
        issue = self.pluginParam.getIssue()
        if len(staMapRe) > 0:
            self.outStaMap["3clear_cluster_schedule_o3_zonal_histogram"] = {
                "field": "\'issue\', \'region_id\', \'level\', \'value\'", "values": []}
            outStaMapReValues = self.outStaMap["3clear_cluster_schedule_o3_zonal_histogram"]["values"]
        ...
        if len(staMap) > 0:
            self.outStaMap["3clear_cluster_schedule_o3_zonal_statistics"] = {
                "field": "\'issue\', \'region_id\', \'MAX\', \'MEAN\', \'MIN\'", "values": []}
            outStaMapValues = self.outStaMap["3clear_cluster_schedule_o3_zonal_statistics"]["values"]
